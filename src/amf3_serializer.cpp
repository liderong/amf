...
namespace
{
	...
	std::string s2utfs(const std::string& strSrc){return wstring_to_utf8(s2ws(strSrc));}
	std::string utfs2s(const std::string& strutf){return ws2s(utf8_to_wstring(strutf)); };
}
namespace ns_amf3
{
...
#define EPOCH_DIFF 116444736000000000 //FILETIME starts from 1601-01-01 UTC, epoch from 1970- 01-01
	...
	ns_amf3::AmfObjectHandle AMFObject::addChildWithType( AMFDataType type,const char* name/*=""*/ )
	{
		AmfObjectHandle obj = AMFObject::Alloc();
	...
	void write_obj(AMFContext* ctx,AmfObjectHandle obj)
	{
		int objref = 0x0B;
		write_u29(ctx,objref);
		write_string(ctx,obj->getClassName());
